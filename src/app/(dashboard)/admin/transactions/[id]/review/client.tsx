  ...
  async function handleApprove() {
    ...
    try {
      ...
      await reviewAndApprove(purchase.id, {
        newTotalPrice: noNegotiation || !hasValue ? null : roundMoney(parsed),
        note: note || null,
      })
          ...
          <p className="text-[12px] text-muted-2">
            ...
            {purchase.warehouseName && (
              <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-panel-alt border border-border-soft px-2 py-0.5 text-[10.5px] font-bold text-muted-foreground align-middle">
                <Warehouse className="w-3 h-3 text-emerald" />
                {purchase.warehouseName}
              </span>
            )}
          </p>
        ...
        <div className="space-y-4 rounded-xl border border-border bg-card p-4 xl:sticky xl:top-2">
          ...
          <div>
            ...
            <input
              ...
              value={newTotal}
              disabled={noNegotiation}
              onChange={(e) => setNewTotal(e.target.value.replace(/\D/g, ""))}
              className="w-full p-2 bg-panel-alt border border-border-soft text-foreground text-sm font-mono font-bold rounded-lg outline-none disabled:opacity-40 disabled:cursor-not-allowed"
            />
            ...
            <label className="mt-2 flex items-center gap-2 cursor-pointer select-none">
              <button
                type="button"
                role="switch"
                aria-checked={noNegotiation}
                onClick={() => {
                  setNoNegotiation((v) => !v)
                  if (!noNegotiation) setNewTotal("")
                }}
                className={`relative h-5 w-9 rounded-full border transition-colors cursor-pointer ${
                  noNegotiation ? "bg-amber/60 border-amber/60" : "bg-panel border-border-soft"
                }`}
              >
                <span
                  className={`absolute top-0.5 left-0.5 h-3.5 w-3.5 rounded-full bg-foreground transition-transform ${
                    noNegotiation ? "translate-x-4" : ""
                  }`}
                />
              </button>
              <span className="text-[11px] font-bold text-muted-foreground">Tanpa negosiasi</span>
            </label>
          </div>
                ...
                return gradeSummary.map(([g, { count, net }]) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setGrade(active ? "ALL" : g)
                        setPage(1)
                      }}
                      className={`px-2 py-1 rounded-lg border text-[10.5px] font-mono cursor-pointer transition-colors ${
                        active
